  ...
  return <BrandBrainDialog title="Analyze Brand Data" description="Add existing brand sources to simulate how AI builds a deeper understanding of your brand." onClose={closeSafely} footer={footer}>
    {stage === "input" ? <div className="grid gap-6">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="grid gap-2 text-xs font-extrabold uppercase tracking-[.12em] text-[#657080]">LinkedIn Page<input value={draft.linkedinPage} maxLength={200} onChange={(event) => setDraft((current) => ({ ...current, linkedinPage: event.target.value }))} placeholder="linkedin.com/company/coffeexyz" className="h-11 rounded-lg border border-[#c5d2e5] px-3 text-sm font-medium normal-case tracking-normal outline-none focus:border-[#0058bc] focus:ring-2 focus:ring-blue-100" /></label>
      </div>
...
function draftFromAnalysis(analysis: BrandAnalysis): AnalysisDraft { return { websiteUrl: analysis.sources.websiteUrl, instagramUsername: analysis.sources.instagramUsername, linkedinPage: analysis.sources.linkedinPage, brandGuidelines: previousFile(analysis.sources.brandGuidelinesFileName), companyProfile: previousFile(analysis.sources.companyProfileFileName), productCatalogue: previousFile(analysis.sources.productCatalogueFileName) }; }
function sourcesFromDraft(draft: AnalysisDraft): BrandAnalysisSource { return { websiteUrl: draft.websiteUrl.trim(), instagramUsername: draft.instagramUsername.trim(), linkedinPage: draft.linkedinPage.trim(), brandGuidelinesFileName: draft.brandGuidelines.fileName, companyProfileFileName: draft.companyProfile.fileName, productCatalogueFileName: draft.productCatalogue.fileName }; }
function sourceLabels(draft: AnalysisDraft) { return [[draft.websiteUrl, "Website"], [draft.instagramUsername, "Instagram"], [draft.linkedinPage, "LinkedIn"], [draft.brandGuidelines.fileName, "Brand Guidelines"], [draft.companyProfile.fileName, "Company Profile"], [draft.productCatalogue.fileName, "Product Catalogue"]].filter(([value]) => value.trim()).map(([, label]) => label); }
