...
const platformAssetTypes: Record<string, string[]> = {
...
const editablePlatforms = Object.keys(platformAssetTypes);
...
export function CampaignAssetMap({ initialDays }: { initialDays: CampaignDay[] }) {
  ...
  return (
    <>
      ...
      {selected && <AssetDetailSheet row={selected} onClose={() => setSelectedKey(null)} onMove={moveTo} onDelete={deleteSelected} onSave={(changes) => updateAsset(selected.id, changes)} />}
    </>
  );
}
...
function AssetDetailSheet({ row, onClose, onMove, onDelete, onSave }: { row: WorkflowRow; onClose: () => void; onMove: (status: WorkflowStatus) => void; onDelete: () => void; onSave: (changes: EditableAssetFields) => void }) { return <Sheet onClose={onClose}><div className="flex items-start justify-between border-b border-[#d3e4fe] px-6 py-5"><div><p className="text-[11px] font-extrabold uppercase tracking-[.16em] text-[#717786]">Asset Details</p><h3 className="mt-2 text-2xl font-black text-[#0b1c30]">{modalTitle(row.status)}</h3></div><IconButton label="Close asset details" onClick={onClose}><Close /></IconButton></div><div className="flex flex-1 flex-col gap-5 overflow-y-auto p-6"><StatusBadge status={row.status} /><Info label="Day" value={`Day ${row.dayNumber}`} /><Info label="Date" value={formatScheduleDate(row.scheduledDate)} />{row.status === "BLUEPRINT" ? <AssetEditor row={row} onSave={onSave} /> : <><Info label="Platform" value={row.platform} /><Info label="Asset Type" value={row.assetType} /><Info label="Publish Time" value={row.publishTime} /><Info label="Core Topic" value={row.coreTopic} /><StatusContent row={row} /></>}</div><div className="mt-auto flex flex-wrap gap-2 border-t border-[#d3e4fe] bg-[#f9fbff] p-5">{row.status === "BLUEPRINT" && <SecondaryButton onClick={() => window.dispatchEvent(new CustomEvent("campaign-edit-asset", { detail: row.id }))}>Edit</SecondaryButton>}{row.status === "GENERATING" && <PrimaryButton onClick={() => onMove("READY")}>Complete Mock Generation</PrimaryButton>}{row.status === "READY" && <><PrimaryButton onClick={() => onMove("PUBLISHED")}>Publish Content</PrimaryButton><SecondaryButton>Edit</SecondaryButton><SecondaryButton onClick={() => onMove("ARCHIVED")}>Archive</SecondaryButton></>}{row.status === "PUBLISHED" && <><SecondaryButton>View Post</SecondaryButton><PrimaryButton onClick={() => onMove("ARCHIVED")}>Archive</PrimaryButton></>}{row.status === "ARCHIVED" && <><SecondaryButton>View Archive</SecondaryButton><PrimaryButton onClick={() => onMove("READY")}>Restore</PrimaryButton><DangerButton onClick={onDelete}>Delete</DangerButton></>}</div></Sheet> }
...
function AssetEditor({ row, onSave }: { row: WorkflowRow; onSave: (changes: EditableAssetFields) => void }) {
  ...
  function save() {
    onSave({ platform: platform.trim(), assetType: assetType.trim(), publishTime, coreTopic: coreTopic.trim(), caption: caption.trim() || initialCaption });
    setEditing(false);
  }
...
function EditField({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  ...
  if (isPlatform) return <label><span className="mb-2 block text-[10px] font-extrabold uppercase tracking-[.12em] text-[#717786]">{label}</span><select value={value} onChange={(event) => { const platform = event.target.value; onChange(platform); window.dispatchEvent(new CustomEvent("asset-editor-platform-change", { detail: platform })); }} className="h-11 w-full rounded-lg border border-[#c5d2e5] bg-white px-3 text-sm outline-none focus:border-[#0058bc] focus:ring-2 focus:ring-blue-100">{editablePlatforms.map((platform) => <option key={platform} value={platform}>{platform}</option>)}</select></label>;
...
function ReadyContentMockup({ row }: { row: WorkflowRow }) {
  ...
  return <><div className="overflow-hidden rounded-xl border border-[#d3e4fe] bg-white shadow-sm"><MockPostHeader row={row} published={published} /><GeneratedMedia row={row} compact /><div className="p-4"><div className="mb-3 flex gap-4 text-lg text-[#26384d]"><span>♡</span><span>○</span><span>⌁</span></div><p className="text-xs leading-5 text-[#414755]"><b className="mr-1 text-[#0b1c30]">brandpilot</b>{captionFor(row)}</p><p className="mt-3 text-[10px] font-bold uppercase tracking-[.12em] text-emerald-700">{published ? "Published automatically on schedule" : "Ready · Auto-publish scheduled"}</p></div></div><button type="button" onClick={() => setPreviewOpen(true)} className="mt-4 w-fit rounded-lg border border-[#b9ccec] bg-white px-4 py-2.5 text-sm font-semibold text-[#0058bc] transition hover:bg-[#eff4ff] focus:outline-none focus:ring-2 focus:ring-blue-500">View Post</button>{previewOpen && <GeneratedPostPreview row={row} published={published} onClose={() => setPreviewOpen(false)} />}</>;
}

function GeneratedPostPreview({ row, published, onClose }: { row: WorkflowRow; published: boolean; onClose: () => void }) { return <div className="fixed inset-0 z-[90] flex items-center justify-center overflow-y-auto bg-[#071b33]/80 p-4 backdrop-blur-sm" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section role="dialog" aria-modal="true" aria-label={`Generated ${row.platform} post preview`} className="my-auto w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-[#d3e4fe] px-5 py-4"><div><p className="text-[10px] font-extrabold uppercase tracking-[.16em] text-[#717786]">Generated Post Preview</p><h3 className="mt-1 text-xl font-black text-[#0b1c30]">{row.platform} · {row.assetType}</h3></div><IconButton label="Close generated post preview" onClick={onClose}><Close /></IconButton></div><div className="max-h-[82vh] overflow-y-auto"><MockPostHeader row={row} published={published} /><GeneratedMedia row={row} /><div className="p-5"><div className="mb-4 flex gap-5 text-2xl text-[#26384d]"><span>♡</span><span>○</span><span>⌁</span></div><p className="text-sm leading-6 text-[#414755]"><b className="mr-1 text-[#0b1c30]">brandpilot</b>{captionFor(row)}</p><p className="mt-4 text-[10px] font-extrabold uppercase tracking-[.14em] text-emerald-700">{published ? "Published" : `Scheduled for ${formatScheduleDate(row.scheduledDate)} at ${row.publishTime}`}</p></div></div></section></div>; }
