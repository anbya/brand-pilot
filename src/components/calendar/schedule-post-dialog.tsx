...
import type { ContentCreationSource, ContentObjective, ContentPillar, ContentStatus, SocialPlatform } from "@/lib/calendar/types";

export type SchedulePostPayload = {
  idea: { title: string; coreTopic: string; pillarId: string; objective: ContentObjective; targetAudience: string; mainMessage: string; campaignId?: string; creationSource: ContentCreationSource };
  versions: Array<{ platform: SocialPlatform; assetType: string; headline: string; caption: string; cta: string; hashtags: string[]; mediaUrl?: string; visualBrief?: string; publishDate: string; publishTime: string; timezone: string; status: ContentStatus; createdBy: string }>;
};

type SchedulePostDialogProps = { open: boolean; pillars: ContentPillar[]; defaultDate?: string; onClose: () => void; onSubmit: (payload: SchedulePostPayload) => void };
...
const steps = ["Strategy", "Platforms", "Content & Schedule", "Review"];
...
function createVersion(platform: SocialPlatform, defaultDate: string): VersionDraft {
  return { platform, assetType: platformAssetTypes[platform][0], headline: "", caption: "", cta: "", hashtags: "", mediaUrl: "", visualBrief: "", publishDate: defaultDate, publishTime: "09:00", timezone: "Asia/Jakarta", status: "scheduled", createdBy: "Wanda" };
}

export function SchedulePostDialog({ open, pillars, defaultDate = "2026-07-01", onClose, onSubmit }: SchedulePostDialogProps) {
  const titleId = useId();
  const headingRef = useRef<HTMLHeadingElement>(null);
  const [step, setStep] = useState(0);
  const [strategy, setStrategy] = useState<StrategyDraft>({ title: "", coreTopic: "", pillarId: pillars[0]?.id ?? "", objective: "educate", targetAudience: "", mainMessage: "", campaignId: "", creationSource: "manual" });
  const [versions, setVersions] = useState<Partial<Record<SocialPlatform, VersionDraft>>>({ instagram: createVersion("instagram", defaultDate) });
  ...
  function updateStrategy<Key extends keyof StrategyDraft>(key: Key, value: StrategyDraft[Key]) { setStrategy((current) => ({ ...current, [key]: value })); clearError(`strategy-${key}`); }
  function updateVersion<Key extends keyof VersionDraft>(platform: SocialPlatform, key: Key, value: VersionDraft[Key]) { setVersions((current) => ({ ...current, [platform]: current[platform] ? { ...current[platform], [key]: value } : current[platform] })); clearError(`${platform}-${key}`); }
  function focusField(id: string) { window.requestAnimationFrame(() => document.getElementById(id)?.focus()); }

  function validateStrategy() {
    const next: Errors = {};
    const required: Array<[keyof StrategyDraft, string]> = [["title", "Content title"], ["coreTopic", "Core topic"], ["pillarId", "Content pillar"], ["objective", "Objective"], ["targetAudience", "Target audience"], ["mainMessage", "Main message"], ["creationSource", "Creation source"]];
  ...
  function validateVersions() {
    const next: Errors = {};
    for (const platform of selectedPlatforms) { const draft = versions[platform]; if (!draft) continue; for (const key of ["assetType", "headline", "caption", "cta", "publishDate", "publishTime", "timezone", "status", "createdBy"] as const) if (!String(draft[key]).trim()) next[`${platform}-${key}`] = `${formatAssetTypeLabel(key)} is required.`; if (!validateOptionalUrl(draft.mediaUrl ?? "")) next[`${platform}-mediaUrl`] = "Enter a valid URL including https://."; }
  ...
  return <div role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()} className="fixed inset-0 z-[90] flex items-center justify-center overflow-y-auto bg-[#071b33]/60 p-2 backdrop-blur-[2px] sm:p-5">
    <section role="dialog" aria-modal="true" aria-labelledby={titleId} onMouseDown={(event) => event.stopPropagation()} className="my-auto flex max-h-[96vh] w-full max-w-[900px] flex-col overflow-hidden rounded-xl border border-[#bfd3f2] bg-white shadow-[0_24px_80px_rgba(7,27,51,.28)] sm:max-h-[90vh]">
      <header className="flex shrink-0 items-start justify-between gap-4 border-b border-[#d3e4fe] px-4 py-4 sm:px-7 sm:py-5"><div><h2 ref={headingRef} tabIndex={-1} id={titleId} className="text-xl font-extrabold outline-none sm:text-2xl">Schedule Post</h2><p className="mt-1 text-sm text-[#657080]">Create one idea with tailored versions for each platform.</p></div><button type="button" aria-label="Close Schedule Post dialog" onClick={onClose} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-[#c8d8ef] text-xl outline-none hover:bg-[#eff4ff] focus-visible:ring-2 focus-visible:ring-[#0058bc]">×</button></header>
      <ol aria-label="Schedule post steps" className="flex shrink-0 overflow-x-auto border-b border-[#d3e4fe] bg-[#f8faff] px-4 py-3 sm:px-7">{steps.map((label, index) => <li key={label} aria-current={step === index ? "step" : undefined} className={`flex min-w-fit items-center text-xs font-bold ${step === index ? "text-[#0058bc]" : index < step ? "text-emerald-700" : "text-[#8b96a5]"}`}><span className={`mr-2 flex h-6 w-6 items-center justify-center rounded-full ${step === index ? "bg-[#0058bc] text-white" : index < step ? "bg-emerald-100" : "bg-[#e5eeff]"}`}>{index + 1}</span>{label}{index < steps.length - 1 && <span aria-hidden="true" className="mx-3 h-px w-6 bg-[#c5d2e5] sm:w-10" />}</li>)}</ol>
      ...
      <footer className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-t border-[#d3e4fe] bg-[#f8faff] px-4 py-4 sm:px-7"><button type="button" onClick={onClose} className="rounded-lg px-4 py-2.5 text-sm font-bold text-[#657080] outline-none hover:bg-white focus-visible:ring-2 focus-visible:ring-[#0058bc]">Cancel</button><div className="flex flex-wrap gap-2">{step > 0 && <button type="button" onClick={() => { setErrors({}); setStep((current) => current - 1); }} className="rounded-lg border border-[#c5d2e5] bg-white px-5 py-2.5 text-sm font-bold outline-none hover:bg-[#eff4ff] focus-visible:ring-2 focus-visible:ring-[#0058bc]">Back</button>}{step < 3 ? <button type="button" onClick={nextStep} className="rounded-lg bg-[#0058bc] px-5 py-2.5 text-sm font-bold text-white outline-none hover:bg-[#004493] focus-visible:ring-2 focus-visible:ring-[#0058bc] focus-visible:ring-offset-2">Next</button> : <button type="button" onClick={submit} className="rounded-lg bg-[#0058bc] px-5 py-2.5 text-sm font-bold text-white outline-none hover:bg-[#004493] focus-visible:ring-2 focus-visible:ring-[#0058bc] focus-visible:ring-offset-2">Save Posts</button>}</div></footer>
    </section>
  </div>;
}
...
function StrategyStep({ strategy, pillars, errors, onChange }: { strategy: StrategyDraft; pillars: ContentPillar[]; errors: Errors; onChange: <Key extends keyof StrategyDraft>(key: Key, value: StrategyDraft[Key]) => void }) {
  return <div><h3 className="text-lg font-extrabold">Content strategy</h3><p className="mt-1 text-sm text-[#657080]">Define the shared strategic foundation for every platform version.</p><div className="mt-5 grid gap-5 sm:grid-cols-2">
    ...
    <Field id="strategy-creationSource" label="Creation Source" error={errors["strategy-creationSource"]}><select id="strategy-creationSource" value={strategy.creationSource} onChange={(event) => onChange("creationSource", event.target.value as ContentCreationSource)} className={fieldClass}>{["manual", "ai", "template", "repurposed", "brand-asset"].map((value) => <option key={value} value={value}>{value === "ai" ? "AI" : formatAssetTypeLabel(value)}</option>)}</select></Field>
  </div></div>;
}
...
function ContentStep({ platforms, versions, activePlatform, errors, onActivePlatform, onChange }: { platforms: SocialPlatform[]; versions: Partial<Record<SocialPlatform, VersionDraft>>; activePlatform: SocialPlatform; errors: Errors; onActivePlatform: (platform: SocialPlatform) => void; onChange: <Key extends keyof VersionDraft>(platform: SocialPlatform, key: Key, value: VersionDraft[Key]) => void }) {
  ...
  return <div><div className="flex gap-2 overflow-x-auto pb-2" role="tablist" aria-label="Platform versions">{platforms.map((platform) => <button key={platform} type="button" role="tab" aria-selected={activePlatform === platform} onClick={() => onActivePlatform(platform)} className={`shrink-0 rounded-lg px-4 py-2 text-sm font-bold outline-none focus-visible:ring-2 focus-visible:ring-[#0058bc] ${activePlatform === platform ? "bg-[#0058bc] text-white" : "bg-[#e5eeff] text-[#414755]"}`}>{formatPlatformLabel(platform)}</button>)}</div><div className="mt-5 rounded-xl border border-[#d3e4fe] bg-[#f8faff] p-4 sm:p-5"><div className="flex items-center justify-between"><div><p className="text-xs font-extrabold uppercase tracking-[.1em] text-[#0058bc]">Platform version</p><h3 className="mt-1 text-lg font-extrabold">{formatPlatformLabel(activePlatform)}</h3></div><span className="rounded-full bg-white px-3 py-1 text-xs font-bold text-[#657080]">Asia/Jakarta</span></div><div className="mt-5 grid gap-5 sm:grid-cols-2">
    ...
    <Field id={`${activePlatform}-publishDate`} label="Publish Date" error={error("publishDate")}><input id={`${activePlatform}-publishDate`} type="date" value={draft.publishDate} onChange={(event) => onChange(activePlatform, "publishDate", event.target.value)} aria-invalid={Boolean(error("publishDate"))} className={fieldClass} /></Field>
    <Field id={`${activePlatform}-publishTime`} label="Publish Time" error={error("publishTime")}><input id={`${activePlatform}-publishTime`} type="time" value={draft.publishTime} onChange={(event) => onChange(activePlatform, "publishTime", event.target.value)} aria-invalid={Boolean(error("publishTime"))} className={fieldClass} /></Field>
    <Field id={`${activePlatform}-status`} label="Status" error={error("status")}><select id={`${activePlatform}-status`} value={draft.status} onChange={(event) => onChange(activePlatform, "status", event.target.value as ContentStatus)} className={fieldClass}>{["draft", "ready", "scheduled", "published", "failed"].map((status) => <option key={status} value={status}>{formatAssetTypeLabel(status)}</option>)}</select></Field>
...
function ReviewStep({ strategy, versions, platforms, pillars }: { strategy: StrategyDraft; versions: Partial<Record<SocialPlatform, VersionDraft>>; platforms: SocialPlatform[]; pillars: ContentPillar[] }) {
  ...
  return <div><h3 className="text-lg font-extrabold">Review scheduled posts</h3><p className="mt-1 text-sm text-[#657080]">{platforms.length} platform {platforms.length === 1 ? "version" : "versions"} will be created.</p><section className="mt-5 rounded-xl border border-[#d3e4fe] p-5"><h4 className="text-sm font-extrabold text-[#0058bc]">Idea summary</h4><dl className="mt-4 grid gap-4 text-sm sm:grid-cols-2"><ReviewItem label="Title" value={strategy.title} /><ReviewItem label="Core Topic" value={strategy.coreTopic} /><ReviewItem label="Content Pillar" value={pillar?.name ?? strategy.pillarId} /><ReviewItem label="Objective" value={formatAssetTypeLabel(strategy.objective)} /><ReviewItem label="Target Audience" value={strategy.targetAudience} /><ReviewItem label="Creation Source" value={strategy.creationSource === "ai" ? "AI" : formatAssetTypeLabel(strategy.creationSource)} /><ReviewItem label="Linked Campaign" value={strategy.campaignId || "Not linked"} /></dl></section><div className="mt-4 grid gap-4">{platforms.map((platform) => { const draft = versions[platform]; if (!draft) return null; return <section key={platform} className="rounded-xl border border-[#d3e4fe] bg-[#f8faff] p-5"><h4 className="text-sm font-extrabold text-[#0058bc]">{formatPlatformLabel(platform)}</h4><dl className="mt-4 grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-3"><ReviewItem label="Asset Type" value={formatAssetTypeLabel(draft.assetType)} /><ReviewItem label="Headline" value={draft.headline} /><ReviewItem label="Schedule" value={`${draft.publishDate} at ${draft.publishTime}`} /><ReviewItem label="Timezone" value={draft.timezone} /><ReviewItem label="Status" value={formatAssetTypeLabel(draft.status)} /><ReviewItem label="Created By" value={draft.createdBy} /></dl></section>; })}</div></div>;
}
